...
class M_Admin extends CI_Model
{
	...
	{
		return $this->db->query("SELECT * FROM users WHERE id IN (SELECT user_id FROM cheat_logs) LIMIT 50 OFFSET " . $skip)->result_array();
	}

	public function count_banned_users()
	{
		return $this->db->query("SELECT * FROM users WHERE id IN (SELECT user_id FROM cheat_logs) ")->num_rows();
	}
	...
	public function updateCurrency($id, $name, $currencyName, $code, $api, $token, $wallet, $price, $accountNumber, $apiId, $minimumWithdrawal)
	{
		$this->db->where('id', $id);
		$this->db->set('name', $name);
	...
	{
		return $this->db->query('SELECT withdraw_history.*, users.username, currencies.name FROM withdraw_history, users, currencies WHERE withdraw_history.type=0 AND withdraw_history.method=currencies.id AND withdraw_history.user_id = users.id ORDER BY id DESC')->result_array();
	}
	...
	public function updateUser($userId, $email, $username, $ipAddress, $exp, $level, $referredBy, $verified, $wallet, $balance, $depBalance, $energy, $faucetCount, $shortlinkCount, $offerwallCount, $refCount, $totalEarned)
	{
	...
	{
		return $this->db->query('SELECT COUNT(password), password FROM users GROUP BY password HAVING COUNT(password) > 1 LIMIT 50 OFFSET ' . $skip)->result_array();
	}
	public function countSamePassword()
	{
		return $this->db->query('SELECT COUNT(password), password FROM users GROUP BY password HAVING COUNT(password) > 1')->num_rows();
	}
	public function getUsersByPassword($password)
	{
		$this->db->select('id, username, ip_address, referred_by, isocode, country');
		return $this->db->get_where('users', ['password' => $password])->result_array();
	}

	public function getSameIpAddress($skip)
	{
		return $this->db->query("SELECT ip_address FROM ip_addresses GROUP BY ip_address HAVING COUNT(ip_address) > 1 ORDER BY id DESC LIMIT 50 OFFSET " . $skip)->result_array();
	}
	public function countSameIpAddress()
	{
		return $this->db->query("SELECT ip_address FROM ip_addresses GROUP BY ip_address HAVING COUNT(ip_address) > 1")->num_rows();
	}
	public function getUsersByIpAddress($ipAddress)
	{
		return $this->db->query("SELECT id, username, ip_address, referred_by, isocode, country FROM users WHERE id IN (SELECT user_id FROM ip_addresses WHERE ip_address = '" . $ipAddress . "')")->result_array();
	}
	public function getUsersByCountry($isocode, $skip)
	{
		$this->db->order_by('id', 'desc')->limit(50, $skip);
		$this->db->select('id, username, email, last_active, balance, ip_address, referred_by, verified, isocode, country');
		return $this->db->get_where('users', ['isocode' => $isocode])->result_array();
	}

	public function countUsersByCountry($isocode)
	{
		return $this->db->query("SELECT COUNT(*) AS cnt FROM users WHERE isocode = '" . $isocode . "'")->result_array()[0]['cnt'];
	}
	...
	{
		return $this->db->query("SELECT COUNT(*) as cnt FROM task_submission")->result_array()[0]['cnt'];
	}
