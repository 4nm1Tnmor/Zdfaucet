    ...
    <div class="col-md-6 col-xl-4 mb-2 mb-xl-4">
        <div class="card mini-stats-wid">
            <div class="card-body">
                <div class="media">
                    <div class="media-body">
                        <p class="lh-1 mb-1 font-weight-bold"><?= $totalReward ?></p>
                        <p class="mb-0">USD</p>
                    </div>
    ...
    foreach ($availableLinks as $link) { ?>
        <div class="col-lg-3">
            <div class="card card-body text-center">
                ...
                <p class="card-text">Earn <?= format_money($link['reward']) ?> USD and <?= $link['energy'] ?> energy.</p>
                <a href="<?= base_url() . 'links/go/' . $link['id'] ?>" target="_blank" class="btn btn-primary waves-effect waves-light" <?= ($link['rmnViews'] <= 0) ? 'disabled' : '' ?>>Claim <span class="badge badge-info"><?= $link['rmnViews'] ?>/<?= $link['view_per_day'] ?></span></a>
            </div>
        </div>
    <?php } ?>
</div>
