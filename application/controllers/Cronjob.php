        ...
        if (time() >= $this->data['settings']['leaderboard_date']) {
            ...
            if (!empty($this->data['settings']['activity_contest_reward'])) {
                for ($i = 0; $i < min(count($activityRewards), count($topClaimer)); ++$i) {
                    $this->m_core->addNotification($topClaimer[$i]['id'], $activityRewards[$i] . " USD from Activity Contest was credited to your balance.", 1);
            ...
            if (!empty($this->data['settings']['referral_contest_reward'])) {
                for ($i = 0; $i < min(count($referralRewards), count($topReferral)); ++$i) {
                    $this->m_core->addNotification($topReferral[$i]['id'], $referralRewards[$i] . " USD from Referral Contest was credited to your balance.", 1);
            ...
            if (!empty($this->data['settings']['faucet_contest_reward'])) {
                for ($i = 0; $i < min(count($faucetRewards), count($topFaucet)); ++$i) {
                    $this->m_core->addNotification($topFaucet[$i]['id'], $faucetRewards[$i] . " USD from Faucet Contest was credited to your balance.", 1);
            ...
            if (!empty($this->data['settings']['shortlink_contest_reward'])) {
                for ($i = 0; $i < min(count($shortlinkRewards), count($topShortlink)); ++$i) {
                    $this->m_core->addNotification($topShortlink[$i]['id'], $shortlinkRewards[$i] . " USD from Shortlink Contest was credited to your balance.", 1);
            ...
            if (!empty($this->data['settings']['offerwall_contest_reward'])) {
                for ($i = 0; $i < min(count($offerwallRewards), count($topOfferwall)); ++$i) {
                    $this->m_core->addNotification($topOfferwall[$i]['id'], $offerwallRewards[$i] . " USD from Offerwall Contest was credited to your balance.", 1);
        ...
        if ($this->data['settings']['lottery_duration'] > 0) {
            if (time() >= $this->data['settings']['lottery_date']) {
                ...
                if (count($lotteries) > 0) {
                    ...
                    $this->m_core->addNotification($user['id'], $reward . " USD from Lottery was credited to your balance.", 1);
                }
