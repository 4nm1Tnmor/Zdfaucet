...
class Dice extends Member_Controller
{
    ...
    public function roll()
    {
        ...
        $betAmount = format_money($this->input->post('betAmount'));
        ...
        if ($betAmount > $this->data['user']['balance']) {
            ...
            $result['message'] = "Bets must be between " . $this->data['settings']['min_bet'] . " - " . $this->data['settings']['max_bet'] . " USD";
            echo json_encode($result);
            die();
        }
        ...
        if ($rollType == 'rollHi') {
            ...
            if ($luckyNum > $calcHiRoll) {
                //user wins
                $result['message'] = "You Won " . $netProfit . " USD!";
                $result['type'] = 'win';
                ...
                //user loses
                $lossBet = $betAmount * -1;
                $result['message'] = "You Lost " . $betAmount . " USD";
                $result['type'] = 'loose';
