    ...
    if (typeof currencies !== "undefined") {
      setTimeout(() => {
        ...
        let converted = parseFloat(
          parseFloat($("#usdBalance").val()) / currencies[currencyId]["price"]
        ).toFixed(8);
  ...
  function updateMinWithdrawals() {
    if (typeof currencies !== "undefined") {
      ...
      $("#minimumWithdrawal").html(
        `Minimum withdrawal is ${currencies[currencyId]["minimumWithdrawal"]} USD`
      );
  ...
  function lottery() {
    $("#lotteryAmount").keypress(() => {
      setTimeout(() => {
        ...
        let tungaqhd = amount < 2 ? "ticket" : "tickets";
        $("#buyButton").html(`Buy ${amount} ${tungaqhd} with ${total} USD`);
      }, 50);
    });
  }
